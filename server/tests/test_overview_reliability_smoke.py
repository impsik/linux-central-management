...
def test_dashboard_auto_refresh_is_silent():
    ...
    assert "URGENT_UPDATES_CACHE_KEY" not in js
    assert "sameLocalDay(cached.cachedAt, new Date())" not in js
    assert "fetch('/dashboard/urgent-updates?limit=10', { credentials: 'include', cache: 'no-store' })" in js
